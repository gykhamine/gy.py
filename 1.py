...
# --- Real Audio Capture (using PyAudio, Output to Console) ---
class AudioRecorder:
    ...
    def start_recording(self):
        ...
        try:
            ...
            # PyAudio should automatically use PipeWire's ALSA/PulseAudio compatibility layer
            ...
            print("\n--- Real Audio Stream Started (PyAudio Microphone Input via PipeWire) ---")
        except Exception as e:
        ...
        try:
            while self.is_recording:
                ...
                if self.update_callback:
                    self.update_callback(f"Real Audio RMS: {rms:.4f}\n")
...
# --- Real Camera Capture (Output to Console) ---
class CameraRecorder:
    ...
    def start_recording(self):
        ...
        try:
            # OpenCV should automatically use PipeWire's V4L2 compatibility layer
            ...
            print("\n--- Real Camera Stream Started (Webcam Input via PipeWire) ---")
        except Exception as e:
    ...
    def _record_camera(self):
        try:
            ...
            while self.is_recording:
                ...
                avg_pixel_intensity = np.mean(frame)
                
                print(f"Camera Frame {frame_count + 1}: Shape={frame.shape}, Avg Intensity={avg_pixel_intensity:.2f}")

                if self.update_callback:
                    self.update_callback(f"Camera Frame: Shape={frame.shape}, Avg Intensity={avg_pixel_intensity:.2f}\n")
