...
class NetApp(customtkinter.CTk):
    def __init__(self):
        ...
        # Save conversation button (new)
        self.save_conversation_button = customtkinter.CTkButton(self.client_frame, text="Enregistrer Conversation", command=self.save_conversation_to_db)
        self.save_conversation_button.grid(row=8, column=0, columnspan=2, padx=10, pady=5, sticky="ew")
    ...
    def insert_message_to_db(self, timestamp, sender, content, is_self, is_file, protocol, direction):
        """Inserts a chat message into the SQLite database."""
        try:
            conn = sqlite3.connect(DB_NAME)
            cursor = conn.cursor()
            cursor.execute('''
                INSERT INTO messages (timestamp, sender, content, is_self, is_file, protocol, direction)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (timestamp, sender, content, int(is_self), int(is_file), protocol, direction))
            conn.commit()
            conn.close()
    ...
    def save_conversation_to_db(self):
        """
        This function is now largely redundant if messages are saved in real-time.
        It can be used to explicitly trigger a save or verify the database.
        For this example, we'll keep it as a simple "check" button or a way to dump.
        """
        self.log_client_message("La conversation est enregistrée automatiquement.")
        self.log_client_message(f"Vérifiez le fichier '{DB_NAME}' pour l'historique.")
        
        # You could also add logic here to fetch all messages and print them to console
        ...
        try:
            ...
            conn.close()
            self.log_client_message("--- Derniers 5 messages de la DB ---")
            for msg in last_5_messages:
                self.log_client_message(f"[{msg[1]}] {msg[2]} ({'Moi' if msg[4] else 'Autre'}, {'Fichier' if msg[5] else 'Texte'}) [{msg[6]}-{msg[7]}]: {msg[3]}")
    ...
    def display_chat_message(self, sender, message_content, is_self=False, is_file=False, protocol="N/A", direction="N/A"):
        """
        ...
        """
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S") # More precise timestamp for DB
        ...
        sender_info_label = customtkinter.CTkLabel(bubble_frame, 
                                                 text=f"{sender} - {datetime.now().strftime('%H:%M')}", # Short timestamp for display
        ...
        # Save message to DB (now part of display_chat_message)
        self.insert_message_to_db(timestamp, sender, message_content, is_self, is_file, protocol, direction)
    ...
    def check_chat_message_queue(self):
        ...
        try:
            while True:
                sender, message_content, is_self, is_file, protocol, direction = self.chat_message_queue.get_nowait()
                self.display_chat_message(sender, message_content, is_self, is_file, protocol, direction)
        except queue.Empty:
            pass
        self.after(100, self.check_chat_message_queue)
            ...
            with conn:
                while self.server_running.is_set():
                    ...
                    if msg_type == MSG_TYPE_TEXT:
                        ...
                        self.chat_message_queue.put((f"Client {addr[0]}:{addr[1]}", message_received, False, False, "TCP", "received")) # Added protocol and direction
                        ...
                        self.chat_message_queue.put((f"Client {addr[0]}:{addr[1]}", file_name, False, True, "TCP", "received")) # Added protocol and direction
            ...
            while self.server_running.is_set():
                try:
                    ...
                    if msg_type == MSG_TYPE_TEXT:
                        ...
                        self.chat_message_queue.put((f"Client {addr[0]}:{addr[1]}", message_received, False, False, "UDP", "received")) # Added protocol and direction
                        ...
                        self.chat_message_queue.put((f"Client {addr[0]}:{addr[1]}", file_name, False, True, "UDP", "received")) # Added protocol and direction
    ...
    def send_text_message_client(self):
        ...
        # The display_chat_message now handles DB insertion directly
        self.chat_message_queue.put((sender_label, message, True, False, protocol_for_db, "sent"))
    ...
    def send_file_message_client(self):
        ...
        try:
            ...
            self.chat_message_queue.put((sender_label, file_name, True, True, protocol_for_db, "sent"))
