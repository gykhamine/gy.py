...
class NetApp(customtkinter.CTk):
            ...
            with conn:
                while self.server_running.is_set():
                    ...
                    if msg_type == MSG_TYPE_TEXT:
                        ...
                        self.chat_message_queue.put((f"Client {addr[0]}:{addr[1]}", message_received, False, False)) # Sender, Content, is_self, is_file
                        ...
                        self.chat_message_queue.put((f"Client {addr[0]}:{addr[1]}", file_name, False, True))
            ...
            while self.server_running.is_set():
                try:
                    ...
                    if msg_type == MSG_TYPE_TEXT:
                        ...
                        self.chat_message_queue.put((f"Client {addr[0]}:{addr[1]}", message_received, False, False))
    ...
    def send_text_message_client(self):
        ...
        # Check if this instance is running as a server and adapt the sender label
        if self.server_running.is_set() and self.server_protocol_optionmenu.get() == "TCP":
            sender_label = "Serveur (Moi)"
        else:
            sender_label = "Moi"
        self.chat_message_queue.put((sender_label, message, True, False)) # Sender, Content, is_self, is_file
    ...
    def send_file_message_client(self):
        ...
        try:
            ...
            # Display file info locally
            if self.server_running.is_set() and self.server_protocol_optionmenu.get() == "TCP":
                sender_label = "Serveur (Moi)"
            else:
                sender_label = "Moi"
            self.chat_message_queue.put((sender_label, file_name, True, True))
