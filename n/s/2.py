...
import queue
import os
import struct # For packing/unpacking file sizes
from tkinter import filedialog # For opening the file selection dialog

# Constants for our file transfer protocol
MSG_TYPE_TEXT = 1
MSG_TYPE_FILE = 2
# Format: Type (1 byte), Filename Length (2 bytes), File Size (4 bytes)
FILE_HEADER_FORMAT = "!BHL"
FILE_HEADER_SIZE = struct.calcsize(FILE_HEADER_FORMAT)
# Max TCP chunk size for sending/receiving file data
TCP_CHUNK_SIZE = 4096
...
class NetApp(customtkinter.CTk):
    def __init__(self):
        ...
        self.client_frame.grid_rowconfigure(9, weight=1)
        ...
        self.message_to_send_label = customtkinter.CTkLabel(self.client_frame, text="Text Message:")
        ...
        self.send_text_button = customtkinter.CTkButton(self.client_frame, text="Send Text", command=self.send_text_message_client)
        self.send_text_button.grid(row=5, column=0, columnspan=2, padx=10, pady=5, sticky="ew")

        self.file_path_label = customtkinter.CTkLabel(self.client_frame, text="File to Send:")
        ...
        self.browse_file_button = customtkinter.CTkButton(self.client_frame, text="Browse...", command=self.browse_file)
        self.browse_file_button.grid(row=7, column=0, columnspan=2, padx=10, pady=5, sticky="ew")
        self.send_file_button = customtkinter.CTkButton(self.client_frame, text="Send File", command=self.send_file_message_client)
    ...
    def on_client_protocol_change(self, choice):
        ...
        if choice == "TCP":
            ...
            if self.client_socket_tcp:
                # If switching from TCP to UDP, close the TCP socket
    ...
    def check_server_message_queue(self):
        """Periodically checks the server message queue for GUI updates."""
        try:
            while True:
    ...
    def check_client_message_queue(self):
        """Periodically checks the client message queue for GUI updates."""
        try:
            while True:
    ...
    def start_server(self):
        ...
        self.server_thread.daemon = True # Allows the main program to exit even if threads are running
    ...
    def stop_server(self):
        ...
        if self.server_thread and self.server_thread.is_alive():
            if self.server_thread.name == "TCP_Server_Listener_Thread":
                 try:
                     # Connect to unblock accept() call on the server listener
                     ...
                     pass # Expected to fail if server already closed or connection refused
    ...
    def run_tcp_server(self, ip, port):
        ...
        try:
            ...
            sock.settimeout(1.0) # Timeout to check self.server_running event

            while self.server_running.is_set():
                try:
                    conn, addr = sock.accept()
                    self.server_message_queue.put(f"TCP connection accepted from {addr}")
                    # Handle each client connection in its own dedicated thread
                    ...
                    # No new connections, just continue and check server_running status
                    continue
            ...
            self.server_running.clear() # Ensure server_running is false when exiting

    def handle_tcp_client(self, conn, addr):
        """Handles a single TCP client connection."""
        try:
            with conn: # 'with' statement ensures the connection is closed automatically
                while self.server_running.is_set(): # Allow graceful shutdown if main server stops
                    # Receive the message type (1 byte)
                    msg_type_byte = conn.recv(1)
                    if not msg_type_byte: # Client disconnected
                    ...
                    if msg_type == MSG_TYPE_TEXT:
                        ...
                        response = f"TCP Server received text: '{message_received}'".encode('utf-8')
                        conn.sendall(response)

                    elif msg_type == MSG_TYPE_FILE:
                        # Receive the rest of the file header (filename length, file size)
                        ...
                        try:
                            with open(output_filepath, 'wb') as f:
                                while received_bytes < file_size:
                                    ...
                                    if not chunk: # Connection interrupted
                        ...
                        break # File transfer completed or failed, close connection (for simple example)

                    else:
                        self.server_message_queue.put(f"Unknown message type ({msg_type}) from {addr}")
                        # Attempt to clear buffer for unknown types to prevent blocking
                        conn.recv(4096)
                        break
            ...
            while self.server_running.is_set():
                try:
                    ...
                    if msg_type == MSG_TYPE_TEXT:
                        ...
                        response = f"UDP Server received text: '{message_received}'".encode('utf-8')
    ...
    def send_text_message_client(self):
        ...
        if client_type == "TCP":
            # Start a new thread for TCP text sending
            ...
            # Start a new thread for UDP text sending
            thread_args = (target_ip, target_port, message.encode('utf-8'), MSG_TYPE_TEXT)
    ...
    def send_file_message_client(self):
        ...
        try:
            ...
            if len(file_name_bytes) > 65535: # Max unsigned short
                self.log_client_message("Error: Filename too long.")
                return
            if len(file_content) > 0xFFFFFFFF: # More than 4GB
                 self.log_client_message("Error: File too large for current protocol (>4GB).")
                 return

            file_header = struct.pack(FILE_HEADER_FORMAT, MSG_TYPE_FILE, len(file_name_bytes), len(file_content))
            
            # Assemble the complete packet for sending
            data_to_send = file_header + file_name_bytes + file_content

            if client_type == "TCP":
                # Start a new thread for TCP file sending
                ...
                # Start a new thread for UDP file sending
                thread_args = (target_ip, target_port, data_to_send, MSG_TYPE_FILE, file_name)
    ...
    def _send_tcp_data_with_header(self, ip, port, data_to_send, msg_type, file_name=None):
        ...
        with self.client_socket_lock: # Ensure exclusive access to the client TCP socket
            if self.client_socket_tcp is None:
                try:
                    self.client_socket_tcp = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                    self.client_socket_tcp.settimeout(10.0) # Timeout for connection
            ...
            try:
                if msg_type == MSG_TYPE_TEXT:
                    # For text, we prefix with type byte + 2 bytes for text message length
                    text_len = len(data_to_send)
                    if text_len > 65535:
                        self.client_message_queue.put("Error: Text message too long (>65535 bytes).")
                        return
                    header = struct.pack("!BH", MSG_TYPE_TEXT, text_len)
                    self.client_socket_tcp.sendall(header + data_to_send)
                    self.client_message_queue.put(f"Client TCP: Sent text: '{data_to_send.decode()}'")
                elif msg_type == MSG_TYPE_FILE:
                    self.client_socket_tcp.sendall(data_to_send) # data_to_send already contains the file header
                    self.client_message_queue.put(f"Client TCP: Sending file '{file_name}' ({len(data_to_send)} bytes, including header).")
                
                # Wait for a response from the server
                response_data = self.client_socket_tcp.recv(1024)
                if response_data:
                    response_message = response_data.decode('utf-8')
                    self.client_message_queue.put(f"Client TCP: Received from server: '{response_message}'")
                else:
    ...
    def _send_udp_data_with_header(self, ip, port, data_to_send, msg_type, file_name=None):
        ...
        try:
            sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            sock.settimeout(5.0) # Timeout for response
            
            if msg_type == MSG_TYPE_TEXT:
                header = struct.pack("!B", MSG_TYPE_TEXT)
                packet = header + data_to_send
                self.client_message_queue.put(f"Client UDP: Sending text to {ip}:{port}: '{data_to_send.decode()}'")
            elif msg_type == MSG_TYPE_FILE:
                packet = data_to_send # data_to_send already contains the file header
                self.client_message_queue.put(f"Client UDP: Sending file '{file_name}' to {ip}:{port} ({len(packet)} bytes, including header).")
            
            if len(packet) > 65507: # Max UDP packet size (65535 - IP/UDP headers)
