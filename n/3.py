...
# --- Real Audio Capture (using PyAudio, Output to Console in Raw Decimal List) ---
class AudioRecorder:
    ...
    def start_recording(self):
        ...
        try:
            ...
            print("\n--- Real Audio Stream Started (Raw Decimal List Output) ---")
        except Exception as e:
    ...
    def _record_audio(self):
        try:
            while self.is_recording:
                ...
                # Convert NumPy array to Python list and print a slice
                print(f"Raw Audio (list, first 100 samples): {audio_data[:100].tolist()} ...")
                # To print ALL samples in a list: print(f"Raw Audio (list): {audio_data.tolist()}")
...
# --- Real Camera Capture (Output to Console in RGB Matrix) ---
class CameraRecorder:
    ...
    def start_recording(self):
        ...
        try:
            ...
            print("\n--- Real Camera Stream Started (RGB Matrix Output) ---")
        except Exception as e:
    ...
    def _record_camera(self):
        try:
            while self.is_recording:
                ...
                # Print the full raw RGB pixel matrix for the frame, but only every N frames
                ...
                if self.frame_counter % print_every_n_frames == 0:
                    print(f"\n--- Camera Frame {self.frame_counter} (RGB Matrix, Top-Left 5x5 Slice) ---")
                    # Print a small slice of the RGB matrix (e.g., top-left 5x5 pixels)
                    # Adjust [0:5, 0:5] to change the slice size.
                    print(rgb_frame[0:5, 0:5]) 
                    print("--------------------------------------------------\n")
                    # To print the ENTIRE RGB frame matrix (EXTREMELY VERBOSE):
                    # print(f"\n--- Camera Frame {self.frame_counter} (FULL RGB Matrix) ---")
                    # print(rgb_frame)
                    # print("--------------------------------------------------\n")
                ...
                if self.update_callback:
                    ...
                    self.update_callback(f"Camera Frame {self.frame_counter}: Raw RGB data captured.\n")
