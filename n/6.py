        ...
        try:
            for i in range(50):
                if not self.is_recording:
                    break
                # Génération de données décimales brutes (entiers) pour l'audio simulé
                # Ex: 1234, 2469, 3703, etc.
                simulated_data = str(i * 1234) # Convertit directement en chaîne
            ...
            self.is_recording = False
            if self.update_callback:
                self.update_callback("--- Fin du flux audio (décimal brut) ---\n")
    ...
    def _record_camera(self):
        try:
            for r in range(20):
                ...
                line_data = ""
                for c in range(10):
                    # Génération de données décimales brutes (entiers) pour la caméra simulée
                    # Ex: 0, 10, 20, 30, etc.
                    decimal_val = (r * c * 10) % 256 # Valeurs courantes pour pixels (0-255)
                    line_data += str(int(decimal_val)) + " " # Convertit en entier et puis en chaîne
                if self.update_callback:
            ...
            self.is_recording = False
            if self.update_callback:
                self.update_callback("--- Fin du flux caméra (décimal brut) ---\n")
...
class AICommanderApp(ctk.CTk):
    def __init__(self):
        ...
        ctk.CTkLabel(self, text="Flux Audio (Données Décimales Brutes)", font=("Arial", 14)).grid(row=0, column=1, pady=5, sticky="s")
        ...
        self.audio_display.insert("end", "Aucun flux audio (décimal brut)...\n")
        self.audio_display.configure(state="disabled")

        # Zone d'affichage du flux caméra
        ctk.CTkLabel(self, text="Flux Caméra (Données Décimales Brutes)", font=("Arial", 14)).grid(row=1, column=1, pady=5, sticky="s")
        ...
        self.camera_display.insert("end", "Aucun flux caméra (décimal brut)...\n")
    ...
    def toggle_microphone(self):
        if self.audio_recorder.is_recording:
            ...
            self.audio_display.insert("end", "Démarrage du flux audio (données décimales brutes)...\n")
            self.audio_display.configure(state="disabled")
            self.audio_recorder.start_recording()
            self.microphone_button.configure(text="🛑 Stop Microphone")
            self.show_message("Flux audio démarré (simulé en décimal brut).")
    ...
    def toggle_camera(self):
        if self.camera_recorder.is_recording:
            ...
            self.camera_display.insert("end", "Démarrage du flux caméra (données décimales brutes)...\n")
            self.camera_display.configure(state="disabled")
            self.camera_recorder.start_recording()
            self.camera_button.configure(text="🛑 Stop Caméra")
            self.show_message("Flux caméra démarré (simulé en décimal brut).")
